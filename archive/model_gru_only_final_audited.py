...
# Corrected research-pipeline invariants
SEQUENCE_ALIGNMENT = "anchor_t_uses_[t-seq+1:t+1]"
...
# Model metrics can still be evaluated on every valid 5m anchor.
...
def create_future_log_return_target(
    ...
    # A row-shift is only a valid time horizon when the candles are
    # actually spaced by the requested horizon.  If there is a data gap,
    # do not silently label a 30/60/120 minute jump as a 15-minute target.
    if isinstance(close_series.index, pd.DatetimeIndex):
        expected_delta = pd.Timedelta(minutes=HORIZON_MINUTES)
        actual_delta = close_series.index.to_series().shift(-horizon_steps) - close_series.index.to_series()
...
def create_sequences(
    ...
    آخرین timestep در X مربوط به زمان t است.
...
class LazySequence(tf.keras.utils.Sequence):
    ...
    Instead of materializing N x sequence_length x n_features in RAM,
    batches are assembled only when Keras requests them.
    Anchor indices refer to the local array and the last timestep is t.
    """
    def __init__(self, features_array, target_array, seq_length,
                 anchor_start=None, anchor_end=None, batch_size=BATCH_SIZE,
                 shuffle=False):
        ...
        if len(self.features) != len(self.target):
            raise ValueError("features_array and target_array must have equal length")
        lo = self.seq_length - 1 if anchor_start is None else max(self.seq_length - 1, int(anchor_start))
        hi = len(self.features) - 1 if anchor_end is None else min(len(self.features) - 1, int(anchor_end))
        if hi < lo:
    ...
    def __len__(self):
        return int(np.ceil(len(self.indices) / self.batch_size)) if len(self.indices) else 0

    def __getitem__(self, batch_idx):
        sl = self.indices[batch_idx * self.batch_size:(batch_idx + 1) * self.batch_size]
        anchors = self.anchors[sl]
        X = np.empty((len(anchors), self.seq_length, self.features.shape[1]), dtype=np.float32)
...
def train_model(
    ...
    if isinstance(X_train, tf.keras.utils.Sequence):
        fit_validation = X_val
        fit_y = None
    else:
        fit_validation = (X_val, y_val)
        fit_y = y_train
...
def backtest_strategy(
    ...
    atr_values,
    timestamps=None,
    future_high_paths=None,
    future_low_paths=None,
    future_close_prices=None,
    ...
    Stop/TP (اصلاح‌شده):
        ...
        ATR واقعی هر anchor محاسبه می‌شوند. این یعنی هر معامله فاصله‌ی
        SL/TP مخصوص به خودش را بر اساس نوسان لحظه‌ای بازار دارد.

        در این نسخه هنوز فقط با future horizon return مدل‌سازی شده‌اند
        (نه high/low کندل‌های بین‌راه) - برای بک‌تست دقیق‌تر در آینده باید
        از قیمت‌های tick-level/کندل به کندل بین entry و exit استفاده کرد.
    ...
    if future_high_paths is not None:
        future_high_paths = np.asarray(future_high_paths, dtype=float)
    if future_low_paths is not None:
        future_low_paths = np.asarray(future_low_paths, dtype=float)
    if future_close_prices is not None:
        future_close_prices = np.asarray(future_close_prices, dtype=float)
    ...
    for i in range(
        ...
        current_timestamp = (
            timestamps[i] if timestamps is not None and i < len(timestamps) else None
        )
        if (
            last_entry_timestamp is not None
            and current_timestamp is not None
            and pd.Timestamp(current_timestamp) - pd.Timestamp(last_entry_timestamp)
        ...
        # EXIT RULE
        # -----------------------------------------
        # Prefer the available 5m high/low path. If it is not supplied,
        # fall back to the horizon return so old callers remain compatible.
        exit_reason = "TIME_EXIT"
        realized_move = (actual_return if side == "LONG" else -actual_return)

        if future_high_paths is not None and future_low_paths is not None and i < len(future_high_paths):
            highs = np.asarray(future_high_paths[i], dtype=float)
            lows = np.asarray(future_low_paths[i], dtype=float)
            ...
            directional_return = actual_return if side == "LONG" else -actual_return
...
def generate_walk_forward_folds(
    ...
    """
    Expanding-window walk-forward with one calendar year per validation/test
    block.  When timestamps are available, real calendar boundaries are used
    instead of dividing the row count, so missing candles do not move the
    fold boundaries.

    For a dataset spanning 2018-2026 this yields:
    ...
    The returned endpoints are positional indices into the feature dataframe.
    """
    if n_samples < 1000:
        raise ValueError("Dataset is too small for reliable walk-forward validation.")
    if n_folds < 1:
    ...
    if timestamps is not None:
        ...
        for fold in range(n_folds):
            train_start_ts = pd.Timestamp(year=start_year, month=1, day=1)
            train_end_ts = pd.Timestamp(year=start_year + 3 + fold, month=1, day=1)
    ...
    # Conservative fallback when timestamps are unavailable.  This retains
    # expanding windows and guarantees N_FOLDS when enough rows exist.
...
def prepare_fold_data(
    ...
    # Purge anchors whose future horizon would cross the validation boundary.
    # The original split endpoint is the first validation anchor.
    purged_train_end = max(0, train_end - HORIZON_STEPS)

    train_df = features_df.iloc[:purged_train_end]
    train_target = target_series.iloc[:purged_train_end]
    ...
    purged_val_end = max(val_context_start, val_end - HORIZON_STEPS)
    val_df = features_df.iloc[val_context_start:purged_val_end]
    val_target = target_series.iloc[val_context_start:purged_val_end]
    ...
    usable_test_end = max(test_context_start, test_end - HORIZON_STEPS)
    test_df = features_df.iloc[
        test_context_start:usable_test_end
    ]
...
def main():
    ...
    for fold_idx, (
        ...
        train_end_date = feature_data.index[effective_train_end - 1]
        val_start_date = feature_data.index[val_start]
        val_end_date = feature_data.index[effective_val_end - 1]
        ...
        # ----------------------------------------------------
        # SEQUENCES (LAZY / MEMORY-SAFE)
        # ----------------------------------------------------

        # Train anchors end at the last usable purged train row.
        train_seq = LazySequence(
            train_scaled, train_target.values, SEQUENCE_LENGTH,
            batch_size=BATCH_SIZE, shuffle=False,
        )

        # Validation/test keep historical context, but anchors start at the
        # actual split boundary so no pre-split target is evaluated.
        val_anchor_start = max(SEQUENCE_LENGTH - 1, train_end - val_context_start)
        val_seq = LazySequence(
            val_scaled, val_target.values, SEQUENCE_LENGTH,
            anchor_start=val_anchor_start, batch_size=BATCH_SIZE, shuffle=False,
        )
        test_anchor_start = max(SEQUENCE_LENGTH - 1, val_end - test_context_start)
        test_seq = LazySequence(
            test_scaled, test_target.values, SEQUENCE_LENGTH,
            anchor_start=test_anchor_start, batch_size=BATCH_SIZE, shuffle=False,
        )
        ...
        print(f"[SEQUENCES] Train: {train_seq.n_samples:,}")
        print(f"[SEQUENCES] Val  : {val_seq.n_samples:,}")
        print(f"[SEQUENCES] Test : {test_seq.n_samples:,}")

        if train_seq.n_samples == 0 or val_seq.n_samples == 0 or test_seq.n_samples == 0:
            print("[WARNING] Not enough sequence data. Skipping fold.")
            continue
        ...
        test_anchor_indices = test_seq.anchors.copy()
        y_test_flat = test_seq.target[test_anchor_indices].astype(np.float32, copy=False)
        test_times = test_df.index[test_anchor_indices]
        ...
        # ----------------------------------------------------
        # FUTURE 5m HIGH/LOW PATH FOR BACKTEST
        # ----------------------------------------------------
        aligned_ohlc = data.reindex(feature_data.index)
        future_high_paths = []
        future_low_paths = []
        for gidx in global_test_indices:
            anchor_time = feature_data.index[gidx]
            future_end_time = anchor_time + pd.Timedelta(minutes=HORIZON_MINUTES)
            future_rows = aligned_ohlc.loc[
                (aligned_ohlc.index > anchor_time)
                & (aligned_ohlc.index <= future_end_time)
            ]
            # Do not simulate a full horizon through a gap.
            expected_times = pd.date_range(
                start=anchor_time + pd.Timedelta(minutes=5),
                end=future_end_time,
                freq="5min",
            )
            if len(future_rows) != HORIZON_STEPS or not future_rows.index.equals(expected_times):
                future_high_paths.append(np.full(HORIZON_STEPS, np.nan, dtype=float))
                future_low_paths.append(np.full(HORIZON_STEPS, np.nan, dtype=float))
            else:
                future_high_paths.append(future_rows["high"].to_numpy(dtype=float))
                future_low_paths.append(future_rows["low"].to_numpy(dtype=float))
        ...
        bt = backtest_strategy(
            ...
            future_high_paths=future_high_paths,
            future_low_paths=future_low_paths,
            initial_capital=INITIAL_CAPITAL,
            fee_rate=FEE_RATE,
            slippage_rate=SLIPPAGE_RATE,
            signal_threshold=SIGNAL_THRESHOLD,
        ...
        # Explicitly release fold state before the next walk-forward fold.
        try:
            del model, history, X_train, X_val, X_test, train_seq, val_seq, test_seq
        except Exception:
            pass
